...
initgame:
	; initiates registers to different memory addresses used for
	; different aspects of the game
	mov r4,#0xFF
	mov r6,#0x1
	mov r7,#0x0

	;Register to the balls position
	...
	;Player point left
	mov r8, #(0x20001000 & 0xffff)
	movt r8, #(0x20001000 >> 16)
	str r7, [r8]
	...
	;direction
	mov r10, #(0x20001008 & 0xffff)
	movt r10, #(0x20001008 >> 16)
	str r4, [r10]

	;Servstatus
	mov r11, #(0x2000100c & 0xffff)
	movt r11, #(0x2000100c >> 16)
	str r4, [r11]

	bx lr

leftpoint:
	;Gives the left player a point
	mov r1, #0x80
	str r1, [r5] ; Sets the balls position to left players side
	str r4, [r11] ; Set server status true
	ldrb r0, [r8]
	add r0, r0, #1 ;adds a point to player left
	str r0, [r8]
	bx lr

rightpoint:
	;Gives the right player a point
	mov r1, #0x1
	str r1, [r5] ; Sets the balls position to right players side
	str r4, [r11] ; Set server status true
	ldrb r0, [r9]
	add r0, r0, #1 ; adds a point to player right
	str r0, [r9]
	bx lr

moveball:
	; Moves the ball in the current direction
	ldrb r0, [r11] ; checks server status, if true, skips to move
	cmp r0, #0xFF
	beq end
	ldrb r1, [r5]
	cmp r1, #0x0 ;checks if the ball is out of bounds
	beq outofbounds
	ldr r0, [r5]
	ldrb r3, [r10]
	cmp r3, #0xFF
	beq moveright
	LSL r0, r0, #1 ; Moves the ball one step to the right
	strb r0, [r5]
	b end
moveright:
	LSR r0, r0, #1 ; Moves the ball one step to the left
	strb r0, [r5]
	b end
outofbounds:
	;checks which player won the ball and gives that player a point and serv.
	ldrb r1, [r10]
	cmp r1, #0x0
	beq rightPlayerPoint
	bne leftPlayerPoint
rightPlayerPoint:
	push{lr}
	bl rightpoint
...
intgpiod:
    ...
    str r1, [r0]
    ldr r2, [r5]
    cmp r2, #0x1 ;if right player press button at right time, switch direction
    beq changedirectionD
	push{lr}
	bl leftpoint
	pop{lr}
    b endD
changedirectionD:
	mov r2, #0x0
	str r2, [r10] ; Change direction to move left
	mov r0, #0x00
	str r0, [r11] ; change serv status to false
endD:
	bx lr
...
intgpioe:
    ...
    str r1, [r0]
    ldr r2, [r5]
    cmp r2, #0x80 ;if left player press button at right time, switch direction
    beq changedirectionE
	push{lr}
	bl rightpoint
	pop{lr}
    b endE
changedirectionE:
	mov r2, #0xFF
	str r2, [r10] ; Change direction to move right
	mov r0, #0x00
	str r0, [r11] ; change serv status to false
endE:
	bx lr
