...
;**************************************************************
;
; Sevensegmentsdisplay values for displaying different numbers
;
;**************************************************************
...
initmem:
	...
	mov  r0,#(0x20001004 & 0xffff) ; This memory adress is used for knowing
	movt r0,#(0x20001004 >> 16)    ; which 7-segmentsdisplay and number to display
	strb r1, [r0]				   ; modulo 4 is used decide which of the 4 values to choose from.
	bx lr
...
incrementTenSec: ;increments the value on the ten second number.
	mov r2, #0x0
	strb r2, [r0]
	mov r0, #(0x20001001 & 0xffff)
	movt r0, #(0x20001001 >> 16)
...
incrementMinute: ;increments the value on the minute number.
	mov r2, #0x0
	strb r2, [r0]
	mov r0, #(0x20001002 & 0xffff)
	movt r0, #(0x20001002 >> 16)
	ldrb r1, [r0]
	cmp r1, #0x9
	beq incrementTenMinute
	b  exitwithoutreset

incrementTenMinute: ;increments the value on the ten minute number.
	mov r2, #0x0
	strb r2, [r0]
	mov r0, #(0x20001003 & 0xffff)
	movt r0, #(0x20001003 >> 16)
...
exitwithoutreset: ;increments the current exiting number with 1
	add r1, #0x1
	strb r1, [r0]
	pop{lr}
	bx lr

exitwithreset: ;resets the time to 00:00
	mov r2, #0x0
	strb r2, [r0]
	pop{lr}
	bx lr
...
intgpioe: ;mux
	...
	str r1, [r2] ;decides which seven-segmentsdisplay to display on.

	mov  r3,#(0x20001000 & 0xffff)
	movt r3,#(0x20001000 >> 16) ;points to the first memory adress of the bcd memory adresses;

	add r3, r3, r1 ; depending on the value of our modulo-4 counter, picks out the current number to display.
	ldrb r2, [r3]
	adr r12, SJUSEGTAB
	ldrb r2, [r12, r2]

	mov  r3,#(GPIOB_GPIODATA & 0xffff)
	movt r3,#(GPIOB_GPIODATA >> 16)
	str r2, [r3] ;Displays the number.

	add r1, r1, #0x1 ;Increments the modulo-4 counter with 1 to display the next number on the next sevensegmentsdisplay on the next interrupt.
	cmp r1, #0x4
	beq resetModuloCounter
